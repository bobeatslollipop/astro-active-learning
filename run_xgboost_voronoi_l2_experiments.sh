...
# Shared hyperparameters copied from run_experiments.sh.
...
RESULT_ROOT="${RESULTS_ROOT}/xgboost_voronoi_l2_results_${TOTAL_QUERIES}"
...
python compare_auc_trials.py \
  ...
  --out "${RESULT_ROOT}/figures/final/xgb_voronoi_l2_sampling_auc.png" \
  --labels "XGB kmedian++ + Voronoi-L2" "XGB random + Voronoi-L2" "XGB Wasserstein-L2 + Voronoi-L2" \
  --cmap-runs none

echo ""
echo "All XGBoost Voronoi-L2 sampling experiments completed."
